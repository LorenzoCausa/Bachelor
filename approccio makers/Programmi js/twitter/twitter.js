...
app.post('/send', bodyParser.json(), function(req, res) {
    ...
    if (req.body && req.body.tweet) { 
		tweets.push(req.body);//modifica portentosa di lollo causa
...
app.get('/hashtag/:hashtag', function(req,res)// aggiunta egregia del magnifico te
 {
  function test(p) { return p.hashtag == req.params.hashtag; }
  res.send(tweets.filter(test));
 });
